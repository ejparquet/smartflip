...
type ElectricalJobType = "panel_upgrade" | "rewiring" | "outlet_install" | "lighting" | "emergency" | "inspection" | "ev_charger" | "generator";

interface ElectricalServiceCall extends Omit<ServiceCall, 'type'> {
  type: ServiceCallType;
  electricalType?: ElectricalJobType;
  voltage?: string;
  amperage?: string;
  circuitInfo?: string;
}

const mockElectricalCalls: ElectricalServiceCall[] = [
  {
    id: "1",
    customerId: "c1",
    ...
    longitude: -97.7965,
    type: "emergency",
    electricalType: "panel_upgrade",
    priority: "emergency",
    status: "pending",
    description: "Main panel sparking - power outage",
    issueDetails: "Main breaker panel showing signs of arcing. Half the house has no power. Smoke smell reported.",
    voltage: "240V",
    amperage: "200A",
  ...
  },
  {
    ...
    longitude: -97.6789,
    type: "installation",
    electricalType: "ev_charger",
    priority: "scheduled",
    status: "dispatched",
    description: "EV Charger Installation - Level 2",
    issueDetails: "Install Tesla Wall Connector in garage. Panel has capacity confirmed. Need to run 50A circuit.",
    voltage: "240V",
    amperage: "50A",
  ...
  },
  {
    ...
    longitude: -97.8203,
    type: "installation",
    electricalType: "lighting",
    priority: "scheduled",
    status: "pending",
    description: "Recessed lighting installation",
    issueDetails: "Install 12 recessed LED lights in living room and kitchen. Customer has fixtures ready.",
    voltage: "120V",
  ...
  },
  {
    ...
    longitude: -97.7654,
    type: "repair",
    electricalType: "outlet_install",
    priority: "urgent",
    status: "in_progress",
    description: "Multiple outlets not working",
    issueDetails: "Kitchen outlets on one circuit all dead. GFCI won't reset. Possible circuit issue.",
    voltage: "120V",
    amperage: "20A",
    circuitInfo: "Kitchen GFCI Circuit",
    scheduledDate: new Date().toISOString().split("T")[0],
  ...
  },
  {
    ...
    issueDetails: "Upgrading from 100A to 200A service. Old Federal Pacific panel needs replacement.",
    voltage: "240V",
    amperage: "200A",
  ...
  },
  {
    ...
    longitude: -97.9794,
    type: "inspection",
    electricalType: "inspection",
    priority: "scheduled",
    status: "completed",
    description: "Pre-sale electrical inspection",
    issueDetails: "Full home electrical inspection for real estate transaction.",
...
const electricalTypeConfig: Record<ElectricalJobType, { icon: any; label: string; color: string }> = {
  panel_upgrade: { icon: CircuitBoard, label: "Panel Upgrade", color: "#EF4444" },
  rewiring: { icon: Zap, label: "Rewiring", color: "#272D53" },
  outlet_install: { icon: Plug, label: "Outlets", color: "#3B82F6" },
  lighting: { icon: Lightbulb, label: "Lighting", color: "#FBBF24" },
  emergency: { icon: AlertTriangle, label: "Emergency", color: "#DC2626" },
  inspection: { icon: ClipboardCheck, label: "Inspection", color: "#22C55E" },
  ev_charger: { icon: Gauge, label: "EV Charger", color: "#10B981" },
  generator: { icon: Zap, label: "Generator", color: "#8B5CF6" },
};

export default function ElectricalServiceCallsScreen() {
  const router = useRouter();
  const [serviceCalls, setServiceCalls] = useState<ElectricalServiceCall[]>(mockElectricalCalls);
  ...
  const [selectedCall, setSelectedCall] = useState<ElectricalServiceCall | null>(null);
  ...
  const handleDispatch = useCallback((callId: string) => {
    ...
    Alert.alert("Dispatched!", "You have been dispatched to this electrical job.");
  }, []);
  ...
  const renderServiceCallCard = (call: ElectricalServiceCall) => {
    const priority = priorityConfig[call.priority];
    const status = statusConfig[call.status];
    const electricalType = call.electricalType ? electricalTypeConfig[call.electricalType] : null;
    const PriorityIcon = priority.icon;
    const TypeIcon = electricalType?.icon || Zap;
      ...
      >
        {call.priority === "emergency" && (
          <View style={styles.emergencyBanner}>
            ...
            <Text style={styles.emergencyBannerText}>⚡ ELECTRICAL EMERGENCY</Text>
          </View>
        )}
          ...
          <View style={styles.callMeta}>
            {electricalType && (
              <View style={styles.metaItem}>
                <TypeIcon size={14} color={electricalType.color} />
                <Text style={[styles.metaText, { color: electricalType.color, fontWeight: "600" as const }]}>
                  {electricalType.label}
                </Text>
              </View>
            )}
            {call.voltage && (
              <View style={styles.metaItem}>
                ...
                <Text style={styles.metaText}>{call.voltage}</Text>
              </View>
            )}
            {call.amperage && (
              <View style={styles.metaItem}>
                <Gauge size={14} color={Colors.textSecondary} />
                <Text style={styles.metaText}>{call.amperage}</Text>
              </View>
            )}
            {call.scheduledDate && (
              <View style={styles.metaItem}>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.emergencyAlertContent}>
              ...
              <View style={styles.emergencyAlertText}>
                <Text style={styles.emergencyAlertTitle}>
                  ⚡ {emergencyCalls.length} Electrical Emergency{emergencyCalls.length > 1 ? "s" : ""}
                </Text>
        ...
        <View style={styles.callsSection}>
          <Text style={styles.sectionTitle}>
            Electrical Jobs ({filteredCalls.length})
          </Text>
          ...
          {filteredCalls.length === 0 && (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyStateTitle}>No Electrical Jobs</Text>
        ...
        {selectedCall && (
          <SafeAreaView style={styles.modalContainer}>
            ...
            <ScrollView style={styles.modalContent}>
              ...
              {(selectedCall.voltage || selectedCall.amperage || selectedCall.circuitInfo) && (
                <View style={styles.detailSection}>
                  <Text style={styles.detailLabel}>Electrical Specifications</Text>
                  <View style={styles.specsGrid}>
                    {selectedCall.voltage && (
                      <View style={styles.specItem}>
                        <Zap size={16} color="#EAB308" />
                        <Text style={styles.specLabel}>Voltage</Text>
                        <Text style={styles.specValue}>{selectedCall.voltage}</Text>
                      </View>
                    )}
                    {selectedCall.amperage && (
                      <View style={styles.specItem}>
                        <Gauge size={16} color="#EAB308" />
                        <Text style={styles.specLabel}>Amperage</Text>
                        <Text style={styles.specValue}>{selectedCall.amperage}</Text>
                      </View>
                    )}
                    {selectedCall.circuitInfo && (
                      <View style={[styles.specItem, { flex: 2 }]}>
                        <CircuitBoard size={16} color="#EAB308" />
                        <Text style={styles.specLabel}>Circuit</Text>
                        <Text style={styles.specValue}>{selectedCall.circuitInfo}</Text>
                      </View>
                    )}
                  </View>
                </View>
